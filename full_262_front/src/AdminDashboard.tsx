    ...
    <main className="relative z-10 min-h-[calc(100vh-76px)] bg-slate-950/35 px-4 py-8 text-white sm:px-6">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(280px,0.65fr)]">
          ...
          <article className="rounded-xl border border-slate-700/80 bg-slate-900/70 p-5 backdrop-blur-md"><div className="flex items-center justify-between"><div><h2 className="font-bold">Mais vendidos</h2><p className="mt-1 text-sm text-slate-400">Por quantidade de itens.</p></div><Star className="h-5 w-5 text-orange-300" /></div>{maisVendidos.length > 0 ? <div className="mt-5 h-60" role="img" aria-label="Gráfico de jogos mais vendidos"><ResponsiveContainer width="100%" height="100%"><BarChart data={maisVendidos} layout="vertical" margin={{ top: 0, right: 12, bottom: 0, left: 0 }}><CartesianGrid stroke="#334155" strokeDasharray="3 3" horizontal={false} /><XAxis type="number" allowDecimals={false} tick={{ fill: "#94a3b8", fontSize: 12 }} axisLine={false} tickLine={false} /><YAxis type="category" dataKey="nome" width={118} tick={{ fill: "#e2e8f0", fontSize: 12 }} axisLine={false} tickLine={false} /><Tooltip cursor={{ fill: "rgba(249, 115, 22, 0.12)" }} contentStyle={{ backgroundColor: "#0f172a", border: "1px solid #475569", borderRadius: "8px", color: "#f8fafc" }} /><Bar dataKey="quantidade" name="Vendas" fill="#f97316" radius={[0, 4, 4, 0]} /></BarChart></ResponsiveContainer></div> : <p className="py-6 text-sm text-slate-500">Ainda não há vendas para classificar.</p>}</article>
        </section>
